...
class Query_Builder {
	...
	/**
	 * Translate the given query object and return the query
	 *
	 * @return string
	 */
	public function to_sql( ) {
		return $this->last_query = $this->translateSelect();
	}
	...
	public function insertMultiple($columns, $data, $format = null) {
		...
		foreach ($data as $key => $value) {
			...
			for ($i = 0; $i < count($columns); $i++) {
				$val = isset( $value[$columns[$i]])?  $this->esc_value( $value[$columns[$i]] ) : 'NULL';
				$row .= $val.",";
			}
			$rows[] = rtrim($row, ","). ")";
		}

		$query = "INSERT INTO ". $this->table ." (". implode(",", $columns) .") VALUES ". implode(",", $rows) .";";
		$wpdb->query($query);
	...
	public function updateMultiple($columns, $data, $format = null) {
		global $wpdb;
		...
		foreach ($data as $key => $value) {
			$row = "(";
			for ($i = 0; $i < count($columns); $i++) {
				$val = $this->esc_value( $value[$columns[$i]] );
				$row .= $val.",";
			}
			$rows[] = rtrim($row, ","). ")";
		}
		...
		$wpdb->query("INSERT INTO ". $this->table ." (". implode(",", $columns) .") VALUES ". implode(",", $rows) ." ON DUPLICATE KEY UPDATE ".$duplicates.";");
