...
class Api {
	 ...
	 *
	 *
	 * @return string $token Aaccess token
	 */
	...
	public function get_error() {
		...
		$erro = json_decode($this->last_error['body']);

		error_log(print_r($erro, true));
		
		return empty($erro->detail)? 'generic error from api' : $erro->detail;
	}
	 ...
	 */
	public function get( $url, $args = [], $timeout = 100 ) {
        
		return $this->do_request( 'GET', add_query_arg($args, $url), null, $timeout );
	}
	 ...
	 * @return array|false     Assoc array of API response, decoded from JSON.
	 */
	public function post( $url, $args = [], $timeout = 100 ) {
		return $this->do_request( 'POST', $url, $args, $timeout );
	}
	 ...
	 * @return array|false     Assoc array of API response, decoded from JSON.
	 */
	public function put( $url, $args = [], $timeout = 100 ) {
		return $this->do_request( 'PUT', $url, $args, $timeout );
	}
	 ...
	 * @return array|false     Assoc array of API response, decoded from JSON.
	 */
	public function delete( $url, $args = null, $timeout = 10 ) {
		return $this->do_request( 'DELETE', $url, $args, $timeout );
	}
	 ...
	 */
	private function do_request( $http_verb, $url, $args = null, $timeout = 20 ) {
		$params = [
			'timeout'   => $timeout,
			'method'    => $http_verb,
			'sslverify' => false,
	...
	private function determine_success( $response, $formatted_response ) {
		...
		if ( isset( $formatted_response['error_description'] ) ) {
			$this->last_error = 'Bad Request' === $formatted_response['error_description'] ?
				"Bad request. Please check the code." : $formatted_response['error_description'];
			return;
		}
