...
class Ai1wmpe_PCloud_Curl {
	...
	public function __construct() {
		if ( ! extension_loaded( 'curl' ) ) {
			throw new Ai1wmpe_Error_Exception( __( 'pCloud Extension requires PHP cURL extension. <a href="https://help.servmask.com/knowledgebase/curl-missing-in-php-installation/" target="_blank">Technical Details</a>', AI1WMPE_PLUGIN_NAME ) );
		}
	...
	public function make_request( $parse_as_json = false ) {
		...
		if ( $response === false ) {
			if ( ( $errno = curl_errno( $this->handler ) ) ) {
				throw new Ai1wmpe_Connect_Exception( sprintf( __( 'Unable to connect to pCloud. Error code: %s. <a href="https://help.servmask.com/knowledgebase/pcloud-extension-error-codes/#%s" target="_blank">Technical Details</a>', AI1WMPE_PLUGIN_NAME ), $errno, $errno ) );
			}
		}
		...
		if ( $http_code === 429 ) {
			throw new Ai1wmpe_Rate_Limit_Exception( sprintf( __( 'Too many requests. Please try again later. Error code: %s <a href="https://help.servmask.com/knowledgebase/pcloud-extension-error-codes/#%s" target="_blank">Technical details</a>', AI1WMPE_PLUGIN_NAME ), $http_code, $http_code ) );
		}

		if ( $http_code >= 500 ) {
			throw new Ai1wmpe_Internal_Server_Error_Exception( sprintf( __( 'Internal Server Error. Please try again later. Error code: %s <a href="https://help.servmask.com/knowledgebase/pcloud-extension-error-codes/#%s" target="_blank">Technical details</a>', AI1WMPE_PLUGIN_NAME ), $http_code, $http_code ) );
		}
		...
		if ( $http_code >= 200 ) {
			if ( ( $data = json_decode( $response, true ) ) ) {
				if ( isset( $data['result'] ) && is_int( $data['result'] ) && $data['result'] !== 0 ) {
					if ( isset( $data['error'] ) ) {
						throw new Ai1wmpe_Error_Exception( sprintf( __( '%s. <a href="https://help.servmask.com/knowledgebase/pcloud-extension-error-codes/#%s" target="_blank">Technical details</a>', AI1WMPE_PLUGIN_NAME ), $data['error'], $data['result'] ) );
					} else {
						throw new Ai1wmpe_Error_Exception( sprintf( __( 'Error code: %s. <a href="https://help.servmask.com/knowledgebase/pcloud-extension-error-codes/#%s" target="_blank">Technical details</a>', AI1WMPE_PLUGIN_NAME ), $data['result'], $data['result'] ) );
					}
				}
			}
		}
		...
		if ( $http_code >= 400 ) {
			if ( isset( $this->messages[ $http_code ] ) ) {
				throw new Ai1wmpe_Error_Exception( sprintf( __( '%s. <a href="https://help.servmask.com/knowledgebase/pcloud-extension-error-codes/#%s" target="_blank">Technical details</a>', AI1WMPE_PLUGIN_NAME ), $this->messages[ $http_code ], $http_code ) );
			} else {
				throw new Ai1wmpe_Error_Exception( sprintf( __( 'Error code: %s. <a href="https://help.servmask.com/knowledgebase/pcloud-extension-error-codes/#%s" target="_blank">Technical details</a>', AI1WMPE_PLUGIN_NAME ), $http_code, $http_code ) );
			}
		}
