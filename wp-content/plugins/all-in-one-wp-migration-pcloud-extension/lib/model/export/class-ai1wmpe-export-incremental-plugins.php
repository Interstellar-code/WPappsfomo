...
class Ai1wmpe_Export_Incremental_Plugins {
	...
	public static function execute( $params, Ai1wmpe_PCloud_Client $pcloud = null ) {
		...
		// Set progress
		Ai1wm_Status::info( __( 'Preparing incremental plugin files...', AI1WMPE_PLUGIN_NAME ) );
		...
		if ( ( $incremental_list = ai1wm_open( ai1wm_incremental_plugins_list_path( $params ), 'wb' ) ) ) {
			try {
				if ( ( $response = $pcloud->list_folder( $params['incremental_folder_id'], 'incremental.plugins.list' ) ) ) {
		...
		// Set progress
		Ai1wm_Status::info( __( 'Done preparing incremental plugin files.', AI1WMPE_PLUGIN_NAME ) );
