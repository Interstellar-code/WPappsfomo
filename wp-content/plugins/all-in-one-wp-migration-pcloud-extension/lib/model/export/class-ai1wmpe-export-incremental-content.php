...
class Ai1wmpe_Export_Incremental_Content {
	...
	public static function execute( $params, Ai1wmpe_PCloud_Client $pcloud = null ) {
		...
		if ( ! isset( $params['incremental_folder_id'] ) ) {
			$params['incremental_folder_id'] = get_option( 'ai1wmpe_pcloud_incremental_folder_id', null );
		}

		// Set progress
		Ai1wm_Status::info( __( 'Preparing incremental content files...', AI1WMPE_PLUGIN_NAME ) );
		...
		if ( ( $incremental_list = ai1wm_open( ai1wm_incremental_content_list_path( $params ), 'wb' ) ) ) {
			try {
				if ( ( $response = $pcloud->list_folder( $params['incremental_folder_id'], 'incremental.content.list' ) ) ) {
		...
		// Set progress
		Ai1wm_Status::info( __( 'Done preparing incremental content files.', AI1WMPE_PLUGIN_NAME ) );
