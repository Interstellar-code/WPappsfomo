...
class Ai1wmpe_Export_Incremental_Themes {
	...
	public static function execute( $params, Ai1wmpe_PCloud_Client $pcloud = null ) {
		...
		// Set progress
		Ai1wm_Status::info( __( 'Preparing incremental theme files...', AI1WMPE_PLUGIN_NAME ) );
		...
		if ( ( $incremental_list = ai1wm_open( ai1wm_incremental_themes_list_path( $params ), 'wb' ) ) ) {
			try {
				if ( ( $response = $pcloud->list_folder( $params['incremental_folder_id'], 'incremental.themes.list' ) ) ) {
		...
		// Set progress
		Ai1wm_Status::info( __( 'Done preparing incremental theme files.', AI1WMPE_PLUGIN_NAME ) );
