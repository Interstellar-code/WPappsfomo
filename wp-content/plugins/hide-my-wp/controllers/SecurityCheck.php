...
class HMWP_Controllers_SecurityCheck extends HMWP_Classes_FrontController
{
    ...
    public function getTasks()
    {
        return array(
            ...
            'checkSSL' => array(
                ...
                'solution' => sprintf(esc_html__("Learn how to set your website as %s. %sClick Here%s", 'hide-my-wp'), '<strong>' . str_replace('http:', 'https:', home_url()) . '</strong>', '<a href="'.HMWP_Classes_Tools::getOption('hmwp_plugin_website').'/how-to-move-wordpress-from-http-to-https/" target="_blank">', '</a>'),
            ),
            ...
            'checkConfigChmod' => array(
                ...
                'javascript' => "jQuery(this).hmwp_fixSettings('hmwp_hide_configfile',1);",
            ),
            'checkConfig' => array(
            ...
            if (!is_wp_error($response) ) {
                ...
                if (wp_remote_retrieve_response_code($response) == 200 ) {
                    ...
                    if (HMWP_Classes_Tools::getDefault('hmwp_wp-json') <> HMWP_Classes_Tools::getOption('hmwp_wp-json') ) {
                        $url = home_url() . '/' . HMWP_Classes_Tools::getOption('hmwp_wp-json') . '?hmwp_preview=1' ;
                        $response = HMWP_Classes_Tools::hmwp_localcall($url, array('redirection' => 0, 'cookies' => false));
        ...
        case 'hmwp_fixsettings':
            ...
            if ($name && $value ) {
                if (in_array($name, array_keys(HMWP_Classes_Tools::$options)) ) {
                    ...
                    //Change the chmode for the config file
                    if($name == 'hmwp_hide_configfile'){
                        $wp_dir = trailingslashit( $wp_filesystem->abspath() );
                        $wp_filesystem->chmod( $wp_dir . 'wp-config.php', 0640 );
                    }
    ...
    {
        $url = home_url('wp-config.php?hmwp_preview=1&rnd=' . rand());
        ...
        $url = home_url('wp-config-sample.php?hmwp_preview=1&rnd=' . rand());
        $response = wp_remote_head($url,  array('redirection' => 0, 'timeout' => 5, 'cookies' => false));
    ...
    {
        $url = home_url('readme.html?hmwp_preview=1&rnd=' . rand());
        $response = wp_remote_head($url,  array('redirection' => 0, 'timeout' => 5, 'cookies' => false));
    ...
    {
        $visible = false;
        $url = site_url() . '/' . HMWP_Classes_Tools::getDefault('hmwp_wp-content_url') . '/?rnd=' . rand();
        $response = wp_remote_head($url,  array('redirection' => 0, 'timeout' => 5, 'cookies' => false));
    ...
    {
        $url = home_url() . '/wp-login.php?hmwp_preview=1&rnd=' . rand();
        $response = HMWP_Classes_Tools::hmwp_localcall($url,  array('redirection' => 0, 'cookies' => false));
