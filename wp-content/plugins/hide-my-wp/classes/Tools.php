...
class HMWP_Classes_Tools
{
    ...
    private static function updateDatabase()
    {
        ...
        if(self::$options['hmwp_ver'] < HMWP_VERSION_ID ) {
            ...
            if(self::$options['hmwp_ver'] < 6000) {
                if (!isset(self::$options['hmwp_security_header'])) { self::$options['hmwp_security_header'] = 1;
                }
                if (!isset(self::$options['hmwp_hide_unsafe_headers'])) { self::$options['hmwp_hide_unsafe_headers'] = 1;
                }
                if (!isset(self::$options['hmwp_hide_rsd'])) { self::$options['hmwp_hide_rsd'] = 1;
                }
    ...
    /**
     * Check if user has set Trailing Slash
     * @return bool
     */
    ...
    public static function hmwp_wpcall( $url, $params, $options )
    {
        ...
        if (is_wp_error($response) ) {
            ...
            if(HMWP_Classes_Tools::getValue('hmwp_debug')){
                HMWP_Classes_Error::setError($url . '<br />' . json_encode($options) . '<br />' . $response->get_error_message());
            }
    ...
    public static function emptyCache()
    {
        ...
        try {
            ...
            if (class_exists('\FlyingPress\Purge')){
                \FlyingPress\Purge::purge_entire_cache();
            }
    ...
    public static function checkAccountApi( $token = null, $redirect_to = '' )
    {
        ...
        if(HMWP_Classes_Tools::getValue('hmwp_debug')){
            HMWP_Classes_Error::setError($response);
        }
        ...
        if ($response && $response = json_decode($response, true)) {
            ...
            if (isset($response['error'])) {
                ...
                if(isset($response['message']) && $response['message'] <> '') {
                    ...
                    HMWP_Classes_Error::setError($response['message']);
                }
            ...
            HMWP_Classes_Error::setError(sprintf(esc_html__('CONNECTION ERROR! Make sure your website can access: %s', 'hide-my-wp'), '<a href="' . _HMWP_ACCOUNT_SITE_ . '" target="_blank">' . _HMWP_ACCOUNT_SITE_ . '</a>') . " <br /> ");
        }
