...
class HMWP_Models_Compatibility
{
    ...
    public function checkCompatibility()
    {
        ...
        if (is_admin() || is_network_admin()) {
            ...
            try {
                ...
                //Compatibility with Flying Press plugin
                if (HMWP_Classes_Tools::isPluginActive('flying-press/flying-press.php') ) {

                    if(defined('FLYING_PRESS_SITE_URL')) {
                        add_filter('fp_file_path', function ($file_path, $url) {

                            if (!is_file($file_path)) {
                                $url = HMWP_Classes_ObjController::getClass('HMWP_Models_Files')->getOriginalUrl($url);
                ...
                if (HMWP_Classes_Tools::isPluginActive('swift-performance/performance.php') ||
                    HMWP_Classes_Tools::isPluginActive('swift-performance-lite/performance.php')){

                    add_filter('swift_performance_before_set_cache', array($this, 'findReplaceCache'), PHP_INT_MAX);
                ...
                if (HMWP_Classes_Tools::isPluginActive('wp-rocket/wp-rocket.php') ) {
                    ...
                    //eclude the login and admin from wp-rocket
                    add_filter( 'rocket_cache_reject_uri', function($urls){
                        if (HMWP_Classes_Tools::getDefault('hmwp_admin_url') <> HMWP_Classes_Tools::getOption('hmwp_admin_url')) {
                            $urls[] = '/' . HMWP_Classes_Tools::getOption('hmwp_admin_url');
                        }
                        if (HMWP_Classes_Tools::getDefault('hmwp_login_url') <> HMWP_Classes_Tools::getOption('hmwp_login_url')) {
                            $urls[] = '/' . HMWP_Classes_Tools::getOption('hmwp_login_url');
                        }
                ...
                //Compatibility with XMl Sitemap - tested 09032022
        ...
        add_action(
            'init', function () {
                //Compatibility with iThemes Security, Temporary Login Plugin, LoginPress
        ...
        if (HMWP_Classes_Tools::isPluginActive('ithemes-security-pro/ithemes-security-pro.php') 
            ...
            if (isset($settings['hide-backend']['enabled']) && $settings['hide-backend']['enabled'] ) {
                if (isset($settings['hide-backend']['slug']) && $settings['hide-backend']['slug'] <> '' ) {
                    ...
                    HMWP_Classes_Tools::$options['hmwp_login_url'] = HMWP_Classes_Tools::getDefault('hmwp_login_url');
                }
            }
        }
        ...
        //Compatibility with Uptimate Member - tested 11162021
        ...
        if(HMWP_Classes_Tools::isPluginActive('woocommerce/woocommerce.php')){
            HMWP_Classes_ObjController::getClass('HMWP_Models_Compatibility_Woocommerce');
        }
    ...
    public function checkCacheFiles()
    {
        ...
        if ((HMWP_Classes_Tools::isPluginActive('swift-performance/performance.php') ||
            HMWP_Classes_Tools::isPluginActive('swift-performance-lite/performance.php'))
            && defined('SWIFT_PERFORMANCE_CACHE_DIR')) {
            //Set the cache directory for this plugin
    ...
    public static function getAlerts()
    {
        ...
        if (is_admin() || is_network_admin() ) {
            ...
            if (HMWP_Classes_Tools::isThemeActive('Avada') ) {
                if ((HMWP_Classes_Tools::getOption('hmwp_mode') <> 'default')) {
                    if (defined('FUSION_LIBRARY_URL') && strpos(FUSION_LIBRARY_URL, 'wp-content') !== false && HMWP_Classes_Tools::getOption('hmwp_themes_url')  <> HMWP_Classes_Tools::getDefault('hmwp_themes_url') ) {
                        $avadaPath = false;
