...
class HMWP_Models_Rewrite
{
    ...
    public function getBuffer( $buffer )
    {
        ...
        if (HMWP_Classes_Tools::isAjax()  ) {
            ...
            if(HMWP_Classes_Tools::getOption('hmwp_hideajax_paths') && apply_filters('hmwp_process_buffer', true)) {
    ...
    public function buildRedirect()
    {
        ...
        if (HMWP_Classes_Tools::getOption('hmwp_mode') <> 'default' ) {
            ...
            //Redirect the AJAX
            if (HMWP_Classes_Tools::getDefault('hmwp_admin_url') . '/' . HMWP_Classes_Tools::getDefault('hmwp_admin-ajax_url') <> HMWP_Classes_Tools::getOption('hmwp_admin-ajax_url') 
                && HMWP_Classes_Tools::getDefault('hmwp_admin-ajax_url') <> HMWP_Classes_Tools::getOption('hmwp_admin-ajax_url') 
            ) {
        ...
        if (!empty($this->_replace) ) {
            ...
            if (HMWP_Classes_Tools::isIIS() ) {
                ...
                if (HMWP_Classes_Tools::getOption('hmwp_hide_oldpaths') || HMWP_Classes_Tools::getOption('hmwp_hide_commonfiles')) {
                    add_filter(
                        'hmwp_iis_hide_paths_rules', array(
                        HMWP_Classes_ObjController::getClass('HMWP_Models_Rules'),
        ...
        if (HMWP_Classes_Tools::isIIS() ) {
            ...
            if (!empty($this->_rewrites) ) {
                foreach ( $this->_rewrites as $rewrite ) {
                    ...
                    }elseif (strpos($rewrite['to'], 'index.php') === false && (strpos($rewrite['to'], HMWP_Classes_Tools::getDefault('hmwp_wp-content_url')) !== false || strpos($rewrite['to'], HMWP_Classes_Tools::getDefault('hmwp_wp-includes_url')) !== false)) {
                        if (strpos($rewrite['to'], HMWP_Classes_Tools::getDefault('hmwp_login_url')) === false && strpos($rewrite['to'], HMWP_Classes_Tools::getDefault('hmwp_admin_url')) === false ) {
                            $rewritecode .= 'Source: <strong>^/' . str_replace(array('.css', '.js'), array('\.css', '\.js'), $rewrite['from']) . '</strong> Destination: <strong>' . $home_root . $rewrite['to'] . "</strong> Redirect type: Break;<br />";
                        }
                    }
                }
            ...
            if (!empty($this->_umrewrites)) {
                foreach ( $this->_umrewrites as $rewrite ) {
                    $rewritecode .= 'Source: <strong>^' . str_replace(array('.css', '.js'), array('\.css', '\.js'), $rewrite['from']) . '</strong> Destination: <strong>' . $home_root . $rewrite['to'] . "</strong> Redirect type: <strong>301 Permanent</strong>;<br />";
                }
            }
            ...
            if (!empty($this->_rewrites) ) {
                foreach ( $this->_rewrites as $rewrite ) {
                    if (strpos($rewrite['to'], 'wp-login.php') === false) {
                        $rewritecode .= 'Source: <strong>^/' . str_replace(array('.css', '.js'), array('\.css', '\.js'), $rewrite['from']) . '</strong> Destination: <strong>' . $home_root . $rewrite['to'] . "</strong> Redirect type: <strong>Break</strong>;<br />";
                    }
                }
            }

            if ($rewritecode <> '' ) {
                HMWP_Classes_Error::setError(sprintf(esc_html__('Flywheel detected. Add the redirects in the Flywheel Redirect rules panel %s.', 'hide-my-wp'), '<strong><a href="'.HMWP_Classes_Tools::getOption('hmwp_plugin_website').'/how-to-setup-hide-my-wp-on-flywheel-server/" target="_blank" style="color: red">' . esc_html__("Learn How To Add the Code", 'hide-my-wp') . '</a></strong> <br /><br /><pre>' . $rewritecode . '</pre>' . $form),'notice',false);
    ...
    public function network_admin_url( $url, $path = '', $scheme = null )
    {
        ...
        $from = HMWP_Classes_Tools::getDefault('hmwp_admin_url');
    ...
    public function login_url( $url )
    {
        ...
        if (HMWP_Classes_Tools::getDefault('hmwp_login_url') <> HMWP_Classes_Tools::getOption('hmwp_login_url')
            && strpos($url, HMWP_Classes_Tools::getDefault('hmwp_login_url')) !== false
        ) {
    ...
    public function loopCheck( $url )
    {
        ...
        if (HMWP_Classes_Tools::getOption('hmwp_hide_wplogin') || HMWP_Classes_Tools::getOption('hmwp_hide_login') ) {
            if (HMWP_Classes_Tools::getDefault('hmwp_login_url') <> HMWP_Classes_Tools::getOption('hmwp_login_url') ) {
                ...
                $redirect_url = parse_url($url, PHP_URL_HOST) . parse_url($url, PHP_URL_PATH);
                if ($this->searchInString($redirect_url, $paths) ) {
    ...
    function get404Page( $usetheme = false )
    {
        ...
        try {
            ...
            echo '<html lang="'.get_locale().'"><meta<head><title>'.esc_html__('404 Not Found', 'hide-my-wp').'</title></head><body><h1>'.esc_html__('Page Not Found', 'hide-my-wp').'</h1><p>'.sprintf(esc_html__('The requested URL %s was not found on this server.', 'hide-my-wp') ,((isset($_SERVER['REQUEST_URI'])) ? esc_url($_SERVER['REQUEST_URI']) : '')) . ' </p></body></html>';
            exit();
    ...
    function get403Error()
    {
        ...
        try {
            ...
            echo '<html lang="'.get_locale().'"><meta<head><title>'.esc_html__('403 Forbidden', 'hide-my-wp').'</title></head><body><h1>'.esc_html__('Forbidden', 'hide-my-wp').'</h1><p>'.sprintf(esc_html__("You don't have the permission to access %s on this server.", 'hide-my-wp') ,((isset($_SERVER['REQUEST_URI'])) ? esc_url($_SERVER['REQUEST_URI']) : '')) . ' </p></body></html>';
            exit();
    ...
    public function find_replace( $content)
    {
        if (HMWP_Classes_Tools::doChangePaths() && apply_filters('hmwp_process_find_replace', true) ) {
            ...
            if (is_string($content) && $content <> '') {
                ...
                if (strpos($content, HMWP_Classes_Tools::getDefault('hmwp_wp-content_url')) === false
                    && strpos($content, HMWP_Classes_Tools::getDefault('hmwp_wp-includes_url')) === false
                    && $this->_replaced
                ) {
                    return $content;
                }
    ...
    public function replaceLinks( $found )
    {
        ...
        if (strpos($url, '//') === false && strpos($url, '\/\/') === false ) {
            ...
            if (strpos($url, HMWP_Classes_Tools::getDefault('hmwp_wp-content_url')) !== false
                || strpos($url, HMWP_Classes_Tools::getDefault('hmwp_wp-includes_url')) !== false
