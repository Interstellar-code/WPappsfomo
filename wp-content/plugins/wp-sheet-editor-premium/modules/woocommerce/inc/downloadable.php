	...
	class WPSE_WC_Products_Downloadable {
		...
		function register_columns( $editor ) {
			...
			$editor->args['columns']->register_item(
				...
				array(
					'data_type'               => 'meta_data',
					'column_width'            => 250,
					'title'                   => esc_html__( 'Download files : names', 'vg_sheet_editor' ),
					'supports_formulas'       => true,
					'supports_sql_formulas'   => false,
					'prefetch_meta_key'       => '_downloadable_files',
					...
					'formatted'               => array(
						'comment' => array( 'value' => esc_html__( 'This is optional. Leave empty to use the name from the URLs. Enter multiple names separated by commas', 'vg_sheet_editor' ) ),
					),
				)
			);
			$editor->args['columns']->register_item(
				...
				array(
					'data_type'             => 'meta_data',
					'column_width'          => 250,
					'title'                 => esc_html__( 'Download files : URLs', 'vg_sheet_editor' ),
					'supports_formulas'     => true,
					'supports_sql_formulas' => false,
					'prefetch_meta_key'     => '_downloadable_files',
					...
					'formatted'             => array(
						'comment' => array( 'value' => esc_html__( 'Enter multiple URLs separated by commas', 'vg_sheet_editor' ) ),
					),
				)
			);
			$editor->args['columns']->register_item(
				...
				array(
					...
					'type'                          => 'handsontable',
					'edit_button_label'             => esc_html__( 'Edit files', 'vg_sheet_editor' ),
					'edit_modal_id'                 => 'vgse-download-files',
					'edit_modal_title'              => esc_html__( 'Download files', 'vg_sheet_editor' ),
					'edit_modal_description'        => '<div class="vgse-copy-files-from-product-wrapper"><label>' . esc_html__( 'Copy files from this product: (You need to save the changes afterwards.)', 'vg_sheet_editor' ) . ' </label><br/><select name="copy_from_product" data-remote="true" data-min-input-length="4" data-action="vgse_find_post_by_name" data-post-type="' . VGSE()->WC->post_type . '" data-placeholder="' . esc_html__( 'Select product...', 'vg_sheet_editor' ) . '" class="select2 vgse-copy-files-from-product">
									<option></option>
					...
					'handsontable_column_names'     => array(
						VGSE()->WC->post_type => array( esc_html__( 'Name', 'vg_sheet_editor' ), esc_html__( 'File (url or path)', 'vg_sheet_editor' ) ),
						'product_variation'   => array( esc_html__( 'Name', 'vg_sheet_editor' ), esc_html__( 'File (url or path)', 'vg_sheet_editor' ) ),
					),
		...
		function filter_formula_builder_for_downloadable_files( $settings, $post_type ) {
			...
			$settings['columns_actions']['wc_downloadable_files'] = array(
				...
				'set_value'   =>
				array(
					'description' => esc_html__( 'We will save these files. Existing files will be overwritten. Enter file URL only, you can enter multiple URLs separated by comma.', 'vg_sheet_editor' ),
				),
				'append'      =>
				array(
					'description' => esc_html__( 'We will append the new file to the existing files in the products. Enter file URL only, you can enter multiple URLs separated by comma.', 'vg_sheet_editor' ),
				),
			);
		...
		function get_download_files() {
			...
			if ( empty( $_REQUEST['product_id'] ) ) {
				wp_send_json_error( array( 'message' => esc_html__( 'Please select a product.', 'vg_sheet_editor' ) ) );
			}

			$product_id = (int) VGSE()->helpers->_get_post_id_from_search( sanitize_text_field( wp_unslash( $_REQUEST['product_id'] ) ) );
		...
		function save_download_files() {
			...
			if ( $response ) {
				wp_send_json_success( array( 'message' => esc_html__( 'Files saved.', 'vg_sheet_editor' ) ) );
			}

			wp_send_json_error( array( 'message' => esc_html__( 'The files could not be saved.', 'vg_sheet_editor' ) ) );
		}
