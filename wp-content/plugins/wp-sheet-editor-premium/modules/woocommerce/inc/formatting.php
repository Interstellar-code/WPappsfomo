...
if ( ! class_exists( 'WPSE_WC_Products_Data_Formatting' ) ) {
	...
	class WPSE_WC_Products_Data_Formatting {
		...
		public $wc_subscriptions_after_save_synced = array();
		public $wc_after_save_prices_synced        = array();
		...
		function init() {
			...
			add_action( 'wpse_wc_purge_products_transients', array( $this, 'purge_products_transients' ) );
			add_action( 'vg_sheet_editor/save_rows/after_saving_cell', array( $this, 'product_cell_updated_on_spreadsheet' ), 10, 7 );
			add_filter( 'vg_sheet_editor/save_rows/response', array( $this, 'products_updated_on_spreadsheet' ), 20, 3 );
			add_action( 'vg_sheet_editor/save_rows/after_saving_rows', array( $this, 'maybe_schedule_transients_purge' ), 10, 2 );
			add_action( 'vg_sheet_editor/formulas/execute_formula/after_sql_execution', array( $this, 'maybe_schedule_transients_purge_after_sql_formula' ), 10, 5 );
			add_action( 'vg_sheet_editor/formulas/execute_formula/after_execution', array( $this, 'maybe_schedule_transients_purge_after_formula' ), 10, 5 );
		...
		function notify_if_regular_price_hidden_before_formula( $error, $raw_form_data, $column_settings, $spreadsheet_columns, $post_type, $formula ) {
			...
			if ( $post_type === VGSE()->WC->post_type && strpos( $formula, '=MATH(' ) !== false && strpos( $formula, '$_regular_price$' ) !== false && ! isset( $visible_columns['_regular_price'] ) ) {
				$error = new WP_Error( 'vgse', esc_html__( 'The "Regular price" column is hidden now. Please enable it in the columns manager before running this bulk edit.', 'vg_sheet_editor' ) );
			}
			return $error;
		}
		...
		function _sync_product_lookup_table( $product_id, $modified_data = array() ) {
			...
			$purge_system = VGSE()->get_option( 'wc_product_transients_purge', '' );
		...
		function purge_products_transients( $product_ids ) {
		...
		function maybe_schedule_transients_purge_after_formula( $column, $formula, $post_type, $spreadsheet_columns, $post_ids ) {
			if ( $post_type !== 'product' ) {
				return;
			}
			$purge_system = VGSE()->get_option( 'wc_product_transients_purge' );
			if ( $purge_system !== 'delayed' ) {
				return;
			}
			$first_id = current( $post_ids );
			if ( ! is_int( $first_id ) ) {
				$post_ids = wp_list_pluck( $post_ids, 'ID' );
			}
			$this->schedule_transients_purge( $post_ids );
		}
		function maybe_schedule_transients_purge_after_sql_formula( $column, $formula, $post_type, $spreadsheet_columns, $post_ids ) {
			if ( $post_type !== 'product' ) {
				return;
			}
			$purge_system = VGSE()->get_option( 'wc_product_transients_purge' );
		...
		function maybe_schedule_transients_purge( $data, $post_type ) {
			if ( $post_type !== 'product' ) {
				return;
			}
			$purge_system = VGSE()->get_option( 'wc_product_transients_purge' );
			...
			$ids = array_filter( wp_list_pluck( $data, 'ID' ) );
			$this->schedule_transients_purge( $ids );
		}

		function schedule_transients_purge( $ids ) {
			if ( empty( $ids ) ) {
				return;
			}
			$queue = WC()->queue();
			...
			$existing = $queue->search(
				array(
					'hook'     => 'wpse_wc_purge_products_transients',
					'args'     => array( 'product_ids' => $ids ),
					'status'   => 'pending',
					'group'    => 'wpse_wc_purge_products_transients_group',
					'per_page' => 1,
				)
			);
			if ( ! $existing ) {
				$queue->schedule_single(
					...
					'wpse_wc_purge_products_transients',
					array( 'product_ids' => $ids ),
					'wpse_wc_purge_products_transients_group'
				);
			}
		}
			...
			foreach ( $parent_products as $parent_product ) {
				...
				if ( isset( $parent_product['_sale_price_dates_from'] ) ) {
					if ( ! empty( $parent_product['_sale_price_dates_from'] ) ) {
						$parent_product['_sale_price_dates_from'] = gmdate( 'Y-m-d', strtotime( $parent_product['_sale_price_dates_from'] ) );
					}
				...
				if ( isset( $parent_product['_sale_price_dates_to'] ) ) {
					if ( ! empty( $parent_product['_sale_price_dates_to'] ) ) {
						$parent_product['_sale_price_dates_to'] = gmdate( 'Y-m-d 23:59:59', strtotime( $parent_product['_sale_price_dates_to'] ) );
					}
					...
					foreach ( $products[ $parent_product['ID'] ] as $index => $variation ) {
						...
						if ( isset( $variation['_sale_price_dates_from'] ) ) {
							if ( ! empty( $variation['_sale_price_dates_from'] ) ) {
								$variation['_sale_price_dates_from'] = gmdate( 'Y-m-d', strtotime( $variation['_sale_price_dates_from'] ) );
							}
						...
						if ( isset( $variation['_sale_price_dates_to'] ) ) {
							if ( ! empty( $variation['_sale_price_dates_to'] ) ) {
								$variation['_sale_price_dates_to'] = gmdate( 'Y-m-d 23:59:59', strtotime( $variation['_sale_price_dates_to'] ) );
							}
		...
		function products_updated_on_spreadsheet( $response, $data, $post_type ) {

			if ( ! in_array( $post_type, array( VGSE()->WC->post_type, 'product_variation' ) ) || ! is_array( $data ) ) {
				return $response;
			}
			$update_parent = VGSE()->get_option( 'update_parent_lookup_table_on_variation_edit' );
			$ids           = array_map( 'intval', wp_list_pluck( $data, 'ID' ) );
			foreach ( $data as $item ) {
				if ( $update_parent ) {
					$post = get_post( $item['ID'] );
					if ( $post && $post->post_type === 'product_variation' && ! in_array( (int) $post->post_parent, $ids, true ) ) {
						// Force the parent to be updated so we send _stock as a modified field
						$this->product_updated_on_spreadsheet( (int) $post->post_parent, array(
							'_stock' => null,
						), null, $post_type );
					}
				}
				$this->product_updated_on_spreadsheet( $item['ID'], $item, null, $post_type );
			}
			return $response;
		}
